    ...
    connection.on('message', function(message) {
        if (message.type === 'utf8') {
            ...
            if (json.messageType == 'ServiceResponse') {
                    ...
                    if (json.list.length != 0) {
                        for (var i = 0; i < json.list.length; i++) {
                            if (!_Locations.includes(json.list[i].locationAddress.locationId)) {
                                ...
                                sendSubscribeRequest("897737"); //test only fangtang
                                ...
                                sendSubscribeRequest_lifecircle("897737"); //eventDTO
                            }
                        }
                    }
                } else {
                ...
                console.log("periodic response-keepalive");
            } else if (json.messageType == 'SubscribeResponse') //Subscribe renewal
            {
                setTimeout(sendGetLocationsRequest, 6000000);
                console.log("100min  susbscribe renewal"); //100min
            } else {
