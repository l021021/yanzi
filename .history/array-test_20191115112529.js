...
for (let i = 1; i < RecordArray.length; i++) { //start from Second element 
    ...
    console.log(t1.toLocaleTimeString() + '-前  ' + t1m.toLocaleTimeString() + '  ' + minDiff + '  ' + Aftert1 + ' ' + Beforet2 + '   ' + t2.toLocaleTimeString() + '-后  ' + t2m.toLocaleTimeString());

    if (RecordArray[i].value == 'in' && RecordArray[i - 1].value == 'in') { //全部=1
        //process head
        var t = t1m - 60 * 1000; //Previous
        var tt = TimeArray[t.toLocaleTimeString()] || 0;
        TimeArray.push(t.toLocaleTimeString(), t1ToNext / 60);
        TimeArray.push(t2m.toLocaleTimeString(), PrevTot2 / 60);
        ...
        //process tail
        TimeArray.push(t2m.toLocaleTimeString(), 1);
    } else { //全部标0
        //process head

        TimeArray.push(t1m.toLocaleTimeString(), 0);
        TimeArray.push(t2m.toLocaleTimeString(), 0);
