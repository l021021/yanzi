...
RecordArray.forEach(element => {
    let mydate = new Date();
    mydate.setTime(element.timeStamp);
    console.log(mydate.toLocaleTimeString()); //to xx:xx:xx
});

for (let i = 1; i < RecordArray.length; i++) {
    const element = array[i];
    var t1, t2, t1m, t1s, t2m, t2s = new Date();
    t1.setTime(RecordArray[i - 1].timeStamp);
    t1m.setTime(RecordArray[i - 1].timeStamp);
    t1m.setMiliseconds(0);
    t1m.setSeconds(0);
    t1m.setMinutes(0);
    t1s.setMiliseconds(0);
    t1s.setSeconds(0);
    //t1s.setMinutes(0);
    ...
    console.log(t1.toLocaleTimeString() + '    ' + t1s.toLocaleTimeString() + '  ' + t1m.toLocaleTimeString())
