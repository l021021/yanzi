    ...
    if (sArray[i].value == 'in' && sArray[i - 1].value == 'in') { //全部=1
        ...
        for (const key in timeArray) { //already exits in Array?
            ...
            if (timeArray[key].timeStamp == t.toLocaleTimeString()) {
                ...
                console.log('----这一分存在！加上头部的数值' + t.toLocaleTimeString() + '   ' + JSON.stringify(timeArray[key]) + '  ' + JSON.stringify(sArray[i]))
        ...
        { //tail会重复？
            ...
            for (const key in timeArray) { //already exits in Array?
                ...
                if (timeArray[key].timeStamp == t.toLocaleTimeString()) {
                    ...
                    console.log('----这一分存在！' + t.toLocaleTimeString() + '   ' + JSON.stringify(timeArray[key]) + '  ' + JSON.stringify(sArray[i]))
                    _RecordExist = true;
