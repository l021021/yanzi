...
// s_r_A = RecordArray.groupby("EUI64 - 90 FD9FFFFEA9505A - 3 - Motion");

for (let i = 1; i < s_r_A.length; i++) { 

    t1.setTime(s_r_A[i - 1].timeStamp); //前一个事件时间
    t1.setMilliseconds(0); //得到整秒
    t1m.setTime(s_r_A[i - 1].timeStamp); //t1m：前一个事件的整分
    t1m.setMilliseconds(0);
    t1m.setSeconds(0); //得到整分

    t2.setTime(s_r_A[i].timeStamp); //当前事件时间
    t2.setMilliseconds(0); //得到整秒
    t2m.setTime(s_r_A[i].timeStamp);
    t2m.setMilliseconds(0);
    t2m.setSeconds(0); //second=0
    timeObj.ID = s_r_A[i].Did; //得到整分
    ...
    if (s_r_A[i].value == 'in' && s_r_A[i - 1].value == 'in') { //全部=1
        console.log(s_r_A[i - 1].value + ' and ' + s_r_A[i].value)
        t.setTime(t1m); //前一整分
        ...
        for (const key in timeArray) { //already exits in Array?
            ...
            if (timeArray[key].timeStamp == t.toLocaleTimeString() && timeArray[key].ID == s_r_A[i].Did) {

                console.log('这一分存在！' + t.toLocaleTimeString() + '   ' + JSON.stringify(timeArray[key]) + '  ' + JSON.stringify(s_r_A[i]))
