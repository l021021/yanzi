...
for (let i = 1; i < sArray.length; i++) {
    ...
    if (sArray[i].value == 'in' && sArray[i - 1].value == 'in') { //全部=1
        console.log('--is a in-in ' + sArray[i - 1].value + ' and ' + sArray[i].value)
        t.setTime(t1m); //前一整分
        ...
        for (const key in timeArray) { //already exits in Array?
            ...
            if (timeArray[key].timeStamp == t.toLocaleTimeString()) {
                ...
                console.log('----这一分存在！加头部的数值' + t.toLocaleTimeString() + '   ' + JSON.stringify(timeArray[key]) + '  ' + JSON.stringify(sArray[i]))
        ...
        if (!_RecordExist) { //这一分不存在
            ...
            console.log('----这一分不存在！加入记录：' + t.toLocaleTimeString())
        ...
        { //tail会重复？
            ...
            for (const key in timeArray) { //already exits in Array?
                ...
                if (timeArray[key].timeStamp == t.toLocaleTimeString()) {
                    ...
                    console.log('----这一分存在！'加尾部shu'z + t.toLocaleTimeString() + '   ' + JSON.stringify(timeArray[key]) + '  ' + JSON.stringify(sArray[i]))
                    _RecordExist = true;
            ...
            if (!_RecordExist) {
                ...
                console.log('加入新尾部记录：' + t2m.toLocaleTimeString());
            }
        ...
        console.log('is not-in-in ' + sArray[i - 1].value + ' and ' + sArray[i].value);
        ...
        for (const key in timeArray) { //already exits in Array?
            ...
            if (timeArray[key].timeStamp == t.toLocaleTimeString()) {
                ...
                console.log('这一分存在！更新头部 ' + t.toLocaleTimeString() + '   ' + JSON.stringify(timeArray[key]) + '  ' + JSON.stringify(sArray[i]))
                _RecordExist = true;
                _ExistValue = timeArray[key].value;
                break;
            }
        ...
        if (!_RecordExist) { //这一分不存在
            ...
            var _timeObj = JSON.parse(JSON.stringify(timeObj));
            timeArray.push(_timeObj); //增加记录
            console.log('这一分不存在！加入新记录：' + t.toLocaleTimeString())
        ...
        { //tail会重复？
            ...
            for (const key in timeArray) { //already exits in Array?
                ...
                if (timeArray[key].timeStamp == t.toLocaleTimeString()) {
                    ...
                    console.log('这一分存在！更新尾部 ' + t.toLocaleTimeString() + '   ' + JSON.stringify(timeArray[key]) + '  ' + JSON.stringify(sArray[i]))
                    _RecordExist = true;
                    _ExistValue = timeArray[key].value;
                    break
                }
