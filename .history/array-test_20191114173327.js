...
for (let i = 1; i < RecordArray.length; i++) { //start from Second element 
    ...
    secAfter = 60 - t1.getSeconds();
    secBefore = t2.getSeconds();


    //var timeDiff=RecordArray[i].timeStamp-RecordArray[i-1].timeStamp;
    console.log(t1.toLocaleTimeString() + '  ' + t1m.toLocaleTimeString() + '  ' + minDiff + '  ' + secAfter + ' ' + secBefore);
    //console.log('   ' + t2.toLocaleTimeString() + '  ' + t2m.toLocaleTimeString());

    if (RecordArray[i].value == 'in') {
        //process head
